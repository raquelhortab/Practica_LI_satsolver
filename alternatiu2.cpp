...
void readClauses( ){
    ...
    conflictes=0;
    conflictes_tots.resize(numVars+1,0);
    
    satisfied.resize(numClauses+1,false);
...
void setLiteralToTrue(int lit){
    ...
    if (lit > 0){
      for(int i = 0; i < positius[lit].size(); ++i) satisfied[positius[lit][i]]=true;
      model[lit] = TRUE;}
    else {
      for(int i = 0; i < negatius[-lit].size(); ++i) satisfied[negatius[-lit][i]]=true;
      model[-lit] = FALSE;	}
...
int getNextDecisionLiteral(){
	...
	for (uint j = 0; j < clauses[i].size(); ++j){
	  if (model[clauses[i][j]] == UNDEF){
	      int n = conflictes_tots[j] + positius[j].size()+ negatius[j].size();
	      if(n>max){ 
		  max = n; lit = j;}
	  }
	}
    ...
    for (uint i = 0; i < model.size(); ++i){
        if (i!=0 and model[i] == UNDEF){
            int n = conflictes_tots[i] + positius[i].size()+ negatius[i].size();
            if(n>max){ 
                max = n; lit = i;}
        }
    }
    if(positius[lit].size() > negatius[lit].size()) return lit;
    else return -lit;
    */
}
